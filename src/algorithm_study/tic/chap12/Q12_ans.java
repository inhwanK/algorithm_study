...
class Architecture implements Comparable<Architecture> {

    private int x;
    private int y;
    private int stuff;

    public Architecture(int x, int y, int stuff) {
        this.x = x;
        this.y = y;
        this.stuff = stuff;
    }
...
public class Q12_ans {
    ...
    public int[][] solution(int n, int[][] build_frame) {
        ...
        // 정렬 수행
        ArrayList<Architecture> ans = new ArrayList<Architecture>();
        for (int i = 0; i < answer.size(); i++) {
            ans.add(new Architecture(answer.get(i).get(0), answer.get(i).get(1), answer.get(i).get(2)));
        }
